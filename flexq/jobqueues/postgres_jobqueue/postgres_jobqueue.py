...
class PostgresJobQueue(JobQueueBase):
    ...
    def _wait_in_queues(self, queues_names: List[str]):
        ...
        for queue_name in queues_names + [
            ...
            channel_names_to_listen.append(
                self.parts_join_char.join(
                    [self.instance_name, queue_name, NotificationTypeEnum.todo]
                )
            )
            if queue_name in queues_names:
                channel_names_to_listen.append(
                    self.parts_join_char.join(
                        [self.instance_name, queue_name, NotificationTypeEnum.abort]
                    )
                )
    ...
    def parse_notification(self, notification: Notify) -> Notification:
        ...
        if instance_name != self.instance_name:
            raise JobQueueException(
                f"strange, got notification from wrong instance {instance_name}: {notification.channel}"
            )
    ...
    def send_notify_to_queue(
        ...
        with psycopg2.connect(self.dsn) as conn:
            ...
            with conn.cursor() as curs:
                channel_name = self.parts_join_char.join(
                    [self.instance_name, queue_name, notification_type]
                )

                curs.execute(f'NOTIFY "{channel_name}", %s;', (str(payload),))
                logging.debug(
                    f"sent notify to channel {channel_name} with payload: {payload}"
                )
